...
namespace LambdaExpressionDay24
{
    public class LambdaExpression
    {
        ...
        public static void CheckNameforRemove(List<Person> listPersonInCity)
        {
            if (listPersonInCity.Remove(e => e.Name == "Mac"))

                Console.WriteLine("Yes, A person having name \"SAM\" exists in our list");
        }
        public static void Skiping_AllRecord_ForAgels_LessThanSixty(List<Person> listPersonsInCity)
        {
            foreach (Person person in listPersonsInCity.FindAll(e => (e.Age < 60)))
            {
                Console.WriteLine("Name :" + person.Name + "\t\tAge: " + person.Age);
            }
